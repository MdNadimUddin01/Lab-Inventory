...
function Product() {
  ...
  return (
    <div className="text-black p-4 sm:p-6 md:p-10">
      <ProductsHeader />
      <SearchFilters
        searchQuery={searchQuery}
        selectedCategory={selectedCategory}
...
const ProductsHeader = () => {
  return (
    <div className="container flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
...
const ProductsList = ({ inventory, expandedItemId, toggleItemExpansion }) => {
  const backendUrl = import.meta.env.VITE_BACKEND_URL;
  ...
  async function handleDelete(id) {
    ...
    try {
      ...
      const { data } = await axios.delete(
        backendUrl + "delete/instrument/" + id,
        {
          data: { token },
        }
      );
  ...
  // Mobile card view for each inventory item
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border mb-4 overflow-hidden">
        ...
        >
          <div className="flex items-center">
            <div className="flex-shrink-0 font-bold h-10 w-10 bg-gray-100 rounded-full flex items-center justify-center text-gray-500">
              {(item.instrumentName[0] + item.instrumentName[1]).toUpperCase()}
            </div>
            <div className="ml-3">
              <div className="text-sm font-medium text-gray-900">
                {item.instrumentName}
        ...
        {isExpanded && (
          <div className="border-t px-4 py-3 space-y-3">
            <div className="flex justify-between items-center">
              <div className="text-xs font-medium text-gray-500">Department/Location</div>
              <div>
                <div className="text-sm text-gray-900 text-right">
                  {item.department}
                </div>
                <div className="text-xs text-gray-500 text-right">{item.location}</div>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center">
              <div className="text-xs font-medium text-gray-500">Maintenance</div>
            ...
            <div className="flex justify-end space-x-3 pt-2">
              <Link 
                to={`editInventoryItem/${item._id}`} 
                className="p-1.5 cursor-pointer rounded-md bg-blue-50 hover:bg-blue-100 transition-colors duration-200 shadow-sm hover:shadow focus:outline-none focus:ring-2 focus:ring-blue-300"
              >
                <Edit size={18} className="text-gray-800" />
              </Link>
              <button 
                ...
                className="p-1.5 cursor-pointer rounded-md bg-white hover:bg-red-50 border border-red-200 transition-colors duration-200 shadow-sm hover:shadow focus:outline-none focus:ring-2 focus:ring-red-300"
              >
  ...
  return (
    <>
      ...
      <div className="hidden min-[1000px]:block bg-white rounded-xl shadow-sm border overflow-auto">
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50 border-b">
              <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
